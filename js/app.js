...
(() => {
  ...
  function openEmployeeColorPicker(employeeId) {
    ...
    els.employeeColorPicker.onchange = async () => {
      try {
        await update(ref(db, `scheduleV2/employees/${employeeId}`), {
          color: els.employeeColorPicker.value,
          updatedAt: nowTs(),
    ...
    groups.forEach(group => {
      ...
      group.employees.forEach(employee => {
        ...
        rowName.innerHTML = `
          ...
          <button type="button" class="employee-color-dot ${canManageEmployee(employee) ? 'is-editable' : ''}" data-employee-id="${escapeHtml(employee.id)}" title="${canManageEmployee(employee) ? 'Изменить цвет сотрудника' : 'Цвет сотрудника'}" style="--employee-color:${escapeHtml(employee.color || defaultEmployeeColor(employee.displayName))}"></button>
        `;
  ...
  async function saveVacation() {
    ...
    const vacation = {
      ...
      color: els.vacationColor.value || employee?.color || '#2d63dd',
  ...
  function openAccessRequestModal() {
    ...
    els.accessRequestName.value = '';
    els.accessRequestComment.value = '';
    renderAccessAdminModal();
    Array.from(els.accessRequestDepartments.querySelectorAll('input[type="checkbox"]')).forEach(input => {
      input.checked = false;
    });
  ...
  async function saveAccessRequest() {
    ...
    const name = els.accessRequestName.value.trim();
